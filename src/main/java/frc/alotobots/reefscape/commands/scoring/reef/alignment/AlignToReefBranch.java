...
/**
 * A command that aligns the robot to a specific reef branch at a given level.
 * This command uses PathPlanner's holonomic drive controller to precisely position
 * the robot relative to the target branch, taking into account the alliance color
 * and maintaining proper orientation throughout the alignment process.
 */
public class AlignToReefBranch extends Command {
    ...
    /**
     * Called when the command is initially scheduled. Calculates the initial target pose
    ...
    /**
     * Called repeatedly when this command is scheduled to run. Handles the alignment
     * process when the robot is within the alignment radius.
     */
    @Override
    public void execute() {
    ...
    /**
     * Called once when the command ends or is interrupted. Stops the robot's movement
     * and logs the final status.
     *
    ...
    public void end(boolean interrupted) {
        swerveDrive.stop();
        Logger.recordOutput("Commands/AlignToReefBranch/Status", interrupted ? "Interrupted" : "Completed");
    }

    /**
     * Returns whether the command should end. The command ends when the robot is either
     * outside the alignment radius or has reached the target position within tolerance.
     *
