...
/**
 * Manages a prioritized list of localization sources and selects the highest priority
 * source that's not in a FAULT state.
 */
public class LocalizationStrategy {
    ...
    /**
     * Updates all sources and selects the highest priority non-FAULT source.
     * Sends the current pose to the registered consumer if available.
     */
    public void update() {
        // Update all sources
        ...
        if (poseConsumer != null && currentActiveSource != null) {
            poseConsumer.accept(
                currentActiveSource.getPose(),
                currentActiveSource.getStandardDeviations());
        }
    }
    ...
    public LocalizationSourceState getStrategyState() {
        return (currentActiveSource != null) ? 
               currentActiveSource.getState() : LocalizationSourceState.FAULT;
    }

    /**
     * Registers a consumer to receive pose updates.
     * Typically this would be the swerveDrive.addVisionMeasurement method.
     *
    ...
    private void logSourceChange(LocalizationSource previousSource,
                                 LocalizationSource newSource) {
